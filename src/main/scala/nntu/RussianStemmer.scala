...
class RussianStemmer extends Stemmer {
  private final val vowels = Set('а', 'е', 'ё', 'и', 'о', 'у', 'ы', 'э', 'ю', 'я')
  ...
  @tailrec
  private def findAfter(l: List[Char])(fn: Char => Boolean): String = l.headOption match {
    case Some(c) => if (fn(c)) l.tail.foldLeft("")(_ + _) else findAfter(l.tail)(fn)
    case None => ""
  }
  ...
  private def internalStem(word: String): String = {
    /**
    ...
    val s2 = removeWordSuffix(s1, List("и")) match {
      case (fs, _) => fs
    }
    ...
    val s4 = removeWordSuffix(s3, List("нн")) match {
      case (f, true) => f
      case (f, false) =>
  ...
  private def repeatableStem(word: String): String = {
    var result = internalStem(word)
    for (i <- 0 to 10) {
      val anotherStem = internalStem(result)
      if (!anotherStem.isEmpty && !anotherStem.equals(result)) {
        result = anotherStem
      }
    }
  ...
  override def stem(word: String): String = if (word.length <= 1) word else repeatableStem(word)
...
object RussianStemmer {
  lazy val instance = new RussianStemmer

  def apply(): RussianStemmer = instance

  def stem(word: String): String = instance.stem(word)
}
